...
class Webenq_Form_QuestionnaireNode_Tab_QuestionsRows extends WebEnq4_Form
{
    ...
    public function loadDefaultDecorators()
    {
        ...
        if (empty($decorators)) {
            ...
            ->addDecorator(array('tbody'=>'HtmlTag'), array('tag' => 'tbody', 'class'=>'questionrows sortable2'))
            ->addDecorator(array('table'=>'HtmlTag'), array('tag' => 'table', 'id'=>'questionrows'));
        }
        return $this;
    }
    ...
    public function init()
    {
        ...
        $newQuestionRow = $this->getSubForm('new');
        foreach($newQuestionRow->getElements() as $element){
            $element->setRequired(false);
        }

        $defaultItemValues=new Webenq_Model_QuestionnaireQuestion();
        $newQuestionRow->setDefaults($defaultItemValues->toArray());
        $newQuestionRow->addDecorator('HtmlTag', array(
                'tag' => 'tr',
                'class' => 'hidden',
                'id' => 'newitem'
        ));
    ...
    {
        if (isset($defaults['descendants'])){
            foreach ($defaults['descendants'] as $question){
                if (isset($question['Translation'])) {
                    foreach ($question['Translation'] as $lang => $record) {
                        foreach ($record as $field => $value) {
                            if (($field != 'id') && ($field != 'lang')) {
                                $question[$field][$lang] = $value;
                            }
                        }
                    }
                }
                //we need the id.
                $defaults['rows'][$question['id']]=$question;

            }
        }
        // only create subforms if they are not already created
        if (!$this->_questionsAdded){
            foreach ($defaults['rows'] as $key => $item) {
                if (!in_array($key, array('sortable','new'),true)) {
                    $this->addQuestionRow($key);
                }
            }
            $this->_questionsAdded = true;
        }
